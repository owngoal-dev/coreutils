...
# The package version tracks the Cargo workspace version, which is what every
# utility prints for --version. Refuse to build a tree that would disagree with
# the .deb.
project_version="$(
    sed -n '/^\[workspace\.package\]/,/^\[/ s/^version[[:space:]]*=[[:space:]]*"\([^"]*\)".*/\1/p' \
        "$work_dir/Cargo.toml" | head -n1
)"
[[ "$project_version" == "$upstream_version" ]] || {
    echo "error: upstream Cargo.toml says version ${project_version:-unknown}, configuration/version.txt says $upstream_version" >&2
