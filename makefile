...
FORK_VERIFIER    := $(ROOT_DIR)/scripts/verify-no-fork.sh
...
# binary backs both — only the layout, the architecture label, and whether
# RootHide's symredirect rewrites the path imports differ. Nothing in the tree
# names a bootstrap path: the binary reads no files of its own (locales are
# compiled in) and the utilities are symlinks relative to their own directory.
...
.PHONY: all help print-version print-upstream print-deb-path set-version \
	bump-upstream follow-upstream rebase-patches check source build verify-no-fork \
	package deb deb-roothide deb-rootless debs checksums install clean
...
help:
	...
	@echo "  build          Cross-compile the multi-call binary for iOS"
	@echo "  verify-no-fork Run the utilities that spawn under lldb, on the host"
...
check:
	...
	@[[ "$(SYMREDIRECT_REF)" =~ ^[0-9a-f]{40}$$ ]] || \
		{ echo "error: SYMREDIRECT_REF must be a full commit sha" >&2; exit 65; }
	@echo "==> patch set"
...
build: source
	...
	@$(MAKE) --no-print-directory verify-no-fork

verify-no-fork: source
	@"$(FORK_VERIFIER)" "$(SRC_DIR)" "$(SCRATCH_DIR)"
