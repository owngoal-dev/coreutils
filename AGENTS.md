...
- **This package never replaces the bootstrap's GNU coreutils.** `apt`, `dpkg`,
  Sileo, `darwintools`, `fff` and `wptc-track` all depend on the `coreutils`
  package Procursus ships, and dpkg maintainer scripts run the very tools
  (`rm`, `mv`, `chmod`, `install`, `test`, `sort`) a takeover would swap out. A
  bad swap means apt cannot install the fix. So: no `Provides`, no `Conflicts`,
  no `Replaces`, nothing installed into `usr/bin` but the multi-call binary
  itself, and nothing added to `PATH`. `make check` and `package-deb.sh` both
  refuse a control file that declares any of the three, and `install-device.sh`
  asserts on device that `command -v ls` is still the GNU one and that dpkg
  still attributes it to the `coreutils` package. If a replacing package is
  ever wanted it is a *separate* package, with an epoch version
  (`1:9.5+uutils…`) so `coreutils (>= 8.30-1)` from Sileo still resolves, and
  with the GNU package's `Provides:` aliases (`md5sum sha1sum coreutils-bin
  dirname kill mktemp su`) reproduced.
...
- **Nothing escalates privilege.** Children come from `posix_spawn()` and
  ...
  an identity. `build-ios.sh` rejects a Mach-O importing `setuid`, `seteuid`,
  `setreuid`, `setgid`, `setegid`, `setregid`, `setgroups` or `initgroups`, and
  `patches/0003` drops `chroot` from the feature set because it was the only
  caller (and on iOS it needs root and a root filesystem view the bootstrap
  does not have). Lowering is fine and stays: `nice` reduces its own priority,
  and `timeout` calls `setpgid` so it can signal the child's process group on
  expiry, exactly as GNU does. `_setsid` is an import `std` carries for a
  `Command` option no utility sets.
...
Upstream needs almost nothing to build for iOS: it has 120 `target_vendor =
...
and no bootstrap-path derivation; and it calls `fork()` nowhere. Two patches,
...
not work on iOS anyway. `chcon`/`runcon` need SELinux. `getent` is not a uutils
utility at all — the GNU package on the bootstrap keeps providing it.

## Layout

```
<prefix>/usr/bin/coreutils              the multi-call binary, the only thing in usr/bin
<prefix>/usr/libexec/uutils/bin/<util>  ~106 symlinks -> ../../../bin/coreutils
<prefix>/etc/profile.d/uutils.sh        exports UUTILS_BIN; does not touch PATH
```
...
```sh
make check         # scripts, config, patch set, no-takeover assertion
make debs          # both packages + SHA256SUMS; runs the fork verifier
make install       # install on a device over SSH and smoke-test
```
